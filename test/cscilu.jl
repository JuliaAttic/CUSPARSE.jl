...
function test_cscilu0!(elty)
    ...
    d_A = CUSPARSE.cscilu0!('N',d_A,info,'O')
    h_A = to_host(d_A)
...
function test_cscilu0(elty)
    ...
    d_A = CudaSparseMatrixCSC(A)
    info = CUSPARSE.cscsv_analysis('N','S','U',d_A,'O')
    d_B = CUSPARSE.cscilu0('N',d_A,info,'O')
    h_B = to_host(d_B)
    Alu = lufact(full(A),Val{false})
    Ac = sparse(Alu[:L]*Alu[:U])
...
function test_cscilu02!(elty)
    ...
    @test_throws(DimensionMismatch, CUSPARSE.cscilu02!(d_A,'O'))
end
